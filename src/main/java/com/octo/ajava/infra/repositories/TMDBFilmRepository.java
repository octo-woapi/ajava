...
public class TMDBFilmRepository implements FilmRepository {
  ...
  public List<Film> recupererLesFilms() {
    PaginatedTMDBMovies tmdbResponse = tmdbHttpClient.recupererLesFilmsPopulaires();
    return tmdbFilmMapper.convertirEnFilms(tmdbResponse);
  }

  @Override
  public List<Film> chercherDesFilms(String query) {
    PaginatedTMDBMovies tmdbResponse = tmdbHttpClient.chercherDesFilms(query);
    return tmdbFilmMapper.convertirEnFilms(tmdbResponse);
  }
}
